...
class InformationViewCard : UIView {
    ...
    var binding : Hotel? {
        didSet {
            ...
            DispatchQueue.main.async {
                self.aboutHotelInformation.text = binding.aboutTheHotel.description
                self.tagsCollection.dataStorage = binding.aboutTheHotel.peculiarities.map{ " \($0) "}
    ...
    private let propertiesTableView : MainTableView = {
...
extension InformationViewCard {
    ...
    private func setupUI() {
        ...
        cardView.addSubview(propertiesTableView)
        NSLayoutConstraint.activate([
            propertiesTableView.topAnchor.constraint(equalTo: aboutHotelInformation.bottomAnchor, constant: 16),
            propertiesTableView.leadingAnchor.constraint(equalTo: cardView.leadingAnchor, constant: 16),
            propertiesTableView.trailingAnchor.constraint(equalTo: cardView.trailingAnchor, constant: -16),
            propertiesTableView.heightAnchor.constraint(equalToConstant: 194)
        ])
